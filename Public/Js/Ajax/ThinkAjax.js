...
var ThinkAjax = {
	...
	loading:function (target,tips,effect){
 		...
 		if (elements != null && elements.length > 0)
		{
			...
			firstElement.style.top = (arrayPageScroll[1] +  'px');
			firstElement.style.right = '5px';
			// 显示正在更新
			if ($('loader'))
			{
				var loader_elements = $('loader');
				if(loader_elements.length <= 0)
				{
					loader_elements = $('#loader');
				}
				if(loader_elements.length > 0)
				{
					loader_elements[0].style.display = 'none';
				}
			}
			if ('' != this.image[0])
			{
				firstElement.innerHTML = '<IMG SRC="'+this.image[0]+'"  BORDER="0" ALT="loading..." align="absmiddle"> '+tips;
			}else{
				firstElement.innerHTML = tips;
			}
			//使用更新效果
			var myEffect = firstElement.effects();
