...
{//类定义开始
	...
	private function save($file)
	{
		...
		if($this->checkWindows())//if operate sys is windows
		{
	...
	public function upload($savePath ='')
	{
		...
		//log::write('upload file 1' );
		// 检查上传目录
		if(!is_dir($savePath)) {
			// 检查目录是否编码后的
			if(is_dir(base64_decode($savePath))) {
				$savePath	=	base64_decode($savePath);
				//log::write('upload file 2' );
			}else{
				// 尝试创建目录
				//log::write('upload path  '.$savePath );
				if(!mkdir($savePath)){
		...
		foreach($files as $key => $file) {
			...
			if(!empty($file['name'])) {
				...
				if($this->autoCheck) {
					if(!$this->check($file))
					{
						//log::write('upload file 5' );
						return false;
					}
				}
				
				//log::write('upload file  '.$file );
				//保存上传文件
				if(!$this->save($file))
				{
					//log::write('upload file 6' );
					return false;
				}
...
//			if(eregi('WIN',PHP_OS))
//			{
//				if("GB2312" != mb_detect_encoding($saveName))
//				{
//					$saveName = mb_convert_encoding( $saveName,'GB2312','utf-8');
//				} 
//			}
//			else {
//				if("UTF-8" != mb_detect_encoding($saveName))
//				{
//					$saveName = mb_convert_encoding( $saveName,'UTF-8','GB2312');
//				}
//			}
		}else {
	...
	private function check($file) {
		...
		if(!$this->checkType($file['type'])) {
			...
			//log::write('check file 3' );
			return false;
		}
		//检查文件类型
		if(!$this->checkExt($file['extension'])) {
			$this->error ='上传文件类型不允许';
			//log::write('check file 4' );
			return false;
		}
