	...
	*/
			...
			else
			{
				...
				for	($i=0;$i<count($idcids);$i++)
				{
					...
					$sql = "SELECT class.ICLASSID as CID,class.VNAME AS CLASSNAME,VITEMID as ID, VITEMNAME AS NAME,VITEMCONTENT AS CONTENT
							...
							WHERE items.ICLASSID=class.ICLASSID and items.ICLASSID = '$cid' and items.VITEMID = '$id'";
	...
	public function CustomClassItemInsert()
	{
		if ($this->checkRole())
		{
			...
			$M = new Model();
			$sqlSelect = "select * from THINK_TBCUSTOMITEMS where ICLASSID='$cid' and VITEMID ='$id'";
			...
			$sqlInsert = "INSERT INTO THINK_TBCUSTOMITEMS(ICLASSID,VITEMID,VITEMNAME,VITEMCONTENT,TCOMMENT) 
					values('$cid','$id','$name','$content','$cmt');";
			date_default_timezone_set("Asia/Shanghai");
			$vTime = date("Y-m-d H:i:s");
			$author = $_SESSION['nickname'];    
			$ClassName="";
			$sql = "select ICLASSID,VNAME,VCLASSLEVEL from THINK_TBCUSTOMCLASSES where ICLASSID ='$cid'";
			$nameList=$M->query($sql);
			if (count($nameList)>0) {
	...
	public function CustomClassItemAdd()
	{
		if ($this->checkRole())
		{
			...
			$M = new Model();
			$this->assign('cid', $cid);
			$sql = "select ICLASSID as CID,  VNAME as NAME from THINK_TBCUSTOMCLASSES where ICLASSID = '$cid' ";
			//echo $sql ."<br>";
			//return;
	...
	public function CustomClassItemIndex()
	{
		if ($this->checkRole())
		{
			...
			{
				$M = new Model();
				$sql = "select ICLASSID from THINK_TBCUSTOMCLASSES";
				$list = $M->query($sql);
				if (count($list)>0) {
					$cid=$list[0]['ICLASSID'];
				}
			...
			$sql = "SELECT class.ICLASSID as CID,class.VNAME AS CLASSNAME,VITEMID as ID, VITEMNAME AS NAME,VITEMCONTENT AS CONTENT
					,items.TCOMMENT AS CMT,EDITRECORDID 
					FROM THINK_TBCUSTOMITEMS items,THINK_TBCUSTOMCLASSES class
					WHERE items.ICLASSID=class.ICLASSID and items.ICLASSID = '$cid'";
	...
	public function CustomClassInsert()
	{
		if ($this->checkRole())
		{
			...
			$sqlInsert = "INSERT INTO THINK_TBCUSTOMCLASSES(ICLASSID,VNAME,TCOMMENT,VCLASSLEVEL) VALUES('$cid','$name','$cmt','$level')"; //log::write("CustomClassAdd ->  $sqlInsert");
	...
	public function CustomClassEdit()
	{
		if ($this->checkRole())
		{
			...
			$sql="SELECT ICLASSID as CID, VNAME AS NAME,VCLASSLEVEL AS LEVEL,TCOMMENT AS CMT
					FROM THINK_TBCUSTOMCLASSES where ICLASSID='$cid' ";
			//echo $sql;
			//return;
			$M = new Model();
			$list = $M->query($sql);
