...
class IndexAction extends LoginCommonAction
{
	...
	public function ruleUpdate()
	{
		if ($this->checkRole())
		{
			...
			$filename =$_FILES["rulefile"]["name"];
			//var_dump($_FILES["rulefile"]);
			//return;
			...
			} else
			{
				...
				if(!empty($filename))
				{
					...
					if (!$upload->upload()) {
						//捕获上传异常
						$this->assign('jumpUrl','/Admin/Index/ruleIndex');
						$this->success("发生错误：". $upload->getErrorMsg()."。上传文件失败,正在跳转 ...！");
					}
				}				
				/*
				*	产生修改记录
				*/
				//默认值
				$sqlUpdate = "update THINK_TBCODERULES set VNAME='$name',FILE_NAME='$oldfile',TCOMMENT = '$comment'
						where VID = '$rid'";
				//echo $sqlUpdate;
				//return;
		...
		if ($this->checkRole())
		{
			...
			} else
			{
				...
				for($i=0;$i<count($ridA);$i++)
				{
					...
					$fileNameSystemBased='';//用于在本地查找文件
					$M = new Model();
					$sqlGetFileName = "select FILE_NAME from THINK_TBCODERULES where  VID = '$rid'";
					//echo "sqlGetFileName = $sqlGetFileName";
					
					$nameList = $M->query($sqlGetFileName);
					//var_dump($nameList);
					if(count($nameList)>0)
					{
						$fileNameSystemBased = $nameList[0]['FILE_NAME'];
						//echo "fileNameSystemBased = $fileNameSystemBased <br> 1";
						if($this->checkWindows())
						{
							$fileNameSystemBased=$this->checkGB2312($fileNameSystemBased);
							//echo "fileNameSystemBased = $fileNameSystemBased <br>";
							//return;
						}
						else
						{
					...
					$sqlExcute .= "insert into THINK_TBEDITRECORD(VTIME,VAUTHOR,TCONTENT,EDITID)
							values('$vTime','$author','$content','rule'); ";
					//echo "sqlExcute -> $sqlExcute <br>";
					//echo "path = $path <br>";
					//return;
					///*
					if ( !unlink($path) || !$this->executeSql($sqlExcute) ) {
						//echo "some error! <br>";
						$state=false;
						break;
					}
			...
			} else
			{
				...
				$sqlInsert ="";
				{
					...
					if (count($checkResult)>0) {
						...
						if (C('DB_TYPE') == 'oracle') {
								$sqlInsert="update THINK_TBCODERULES set JLZT='1',VNAME='$name',FILE_NAME = '$filename',TCOMMENT='$comment' 
						...
						if (C('DB_TYPE') == 'oracle') {
								$sqlInsert = "INSERT INTO THINK_TBCODERULES ( VID,VNAME,FILE_NAME ,TCOMMENT,OPERATERID,OPERATERTIME)
										values('$rid','$name','$filename','$comment','$operaterid',sysdate)";							
						}
						else
						{
								$sqlInsert = "INSERT INTO THINK_TBCODERULES ( VID,VNAME,FILE_NAME ,TCOMMENT,OPERATERID,OPERATERTIME)
										values('$rid','$name','$filename','$comment','$operaterid',date('now'))";							
						}
	...
	public function CustomClassEdit()
	{
		if ($this->checkRole())
		{
			...
			$sql="SELECT ICLASSID as CID, VNAME AS NAME,VCLASSLEVEL AS CL,TCOMMENT AS CMT
