...
class UserManagementAction extends Action
{
	...
	public function UserInsert()
	{
		if($this->CheckRole())
		{
			...
			{
				$sqlInsertUser = "INSERT INTO THINK_USER(ACCOUNT,NICKNAME,PASSWORD,EMAIL,OPERATERID,OPERATERTIME,JLZT)
						values('$username','$nickname','$pwdMd5','$email','".$_SESSION['acount']."',date('now'),'1');";
			}
			...
			if ($editor=='on') {
				//数据库不存在的话
				$sqlInsertUser .= "insert into THINK_ROLEUSER(USER_ACOUNT,ROLE_NAME) values('$username','editor');";
				$bRoleSet = true;
			}
			...
			if($this->executeSql($sqlInsertUser))
			{
		...
		if($this->CheckRole())
		{
			...
			*/			
			...
			if ($adminer=='on') {
				//数据库不存在的话
				if (count($list)<=0) {
					$sqlExcute .= "insert into THINK_ROLEUSER(USER_ACOUNT,ROLE_NAME,OPERATERID) values('$username','adminer','$account');";
				}				
			}
			else
			{
			...
			if ($viewer=='on') {
				//数据库不存在的话
				if (count($list)<=0) {
					$sqlExcute .= "insert into THINK_ROLEUSER(USER_ACOUNT,ROLE_NAME,OPERATERID) values('$username','viewer','$account');";
				}				
			}
			else
			{
